...
Task TransmitData(SerialPort senderPort, SerialPort receiverPort)
{
    ...
    {
        Flag = 1,
        SourceAddress = senderPort.PortName == "/dev/ttys001" ? 1 : 2,
        DestinationAddress = senderPort.PortName == "/dev/ttys001" ? 2 : 1,
    ...
    return Task.Run(() => senderPort.Write(stuffedBytes, 0, stuffedBytes.Length));
}
...
void HandleDataReceived(SerialPort port, string portName)
{
    ...
    Console.WriteLine($"  Flag: {packet.Flag}");
    Console.WriteLine($"  Source Address: {packet.SourceAddress}");
    Console.WriteLine($"  Destination Address: {packet.DestinationAddress}");
    Console.WriteLine($"  Data: {Encoding.UTF8.GetString(packet.Data)}");
